...
public class PlayerMovement : MonoBehaviour
{
    ...
    private bool cameraFollowCursor = true;
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.LeftShift) && Sneak)
        {
    ...
    public void ToggleLockCursor(bool locked)
    {
        if (locked)
        {
            cameraFollowCursor = true;
            Cursor.lockState = CursorLockMode.Locked; 
        }
        else
        {
            cameraFollowCursor = false;
    ...
    {
        // se nao for para a camera seguir o cursor
        if (!cameraFollowCursor) { return; }
